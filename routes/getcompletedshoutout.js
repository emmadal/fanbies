...
const getcompletedrequest = express.Router();
 ...
 */
getcompletedrequest.post('/', (req, res) => {
    ...
    jwt.verify(token, process.env.JWT_KEY, (err, loggedin) => {
        ...
        if(loggedin.uid !== null){
            ...
            .then(data =>{
                if(data.length) {
                    res.send({'response':data,'success':true, 'message':'Here are your Completed Request(s).'})
                } else {
                    res.send({'response':data,'success':true, 'message':'You have no Completed Request(s) at the moment.'})
                }
            }).catch((e)=>{
                //sentry
                console.log("😂", e);
                        ...
                        if(results.length !== 0){
                            _.forEach(results,(value,key)=>{
                                ...
                                .then((data)=>{
                                    ...
                                    delete results[key]["recorder_id"];
                                    delete results[key]["owner_id"];
...
module.exports = getcompletedrequest;
