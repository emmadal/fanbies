        ...
        if(loggedin.uid !== null){
            ...
            function getshoutdetails(uresponseid,usertype){
                ...
                return new Promise((resolve,reject)=>{
                    ...
                    const sql=`SELECT ?? FROM ${process.env.DB_PREFIX}shoutout WHERE ${type} = ? AND ( status = ? OR status = ? )`;
                    connection.query(sql,[column,uresponseid,'created', 'processing'],(error, results) => {
